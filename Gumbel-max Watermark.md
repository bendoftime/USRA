...
and, using the convention $0\log 0=0$ (where $x\log x$ is defined at $x=0$ by continuous extension, so that $0\log0=0$),
$$D_{\mathrm{KL}}(P\|Q) =\int_{\mathcal X}\log p\,dP =\int_{\mathcal X}p\log p\,dQ.$$
For $x\ge 0$, define
$$h(x)=x\log x-x+1,$$
where $h(0)=1$. For $x>0$,
$$h'(x)=\log x.$$
...
$$\begin{aligned} D_{\mathrm{KL}}(P\|Q) &=\int_{\mathcal X}p\log p\,dQ\\ &=\int_{\mathcal X}\bigl(p\log p-p+1\bigr)\,dQ +\int_{\mathcal X}(p-1)\,dQ\\ &=\int_{\mathcal X}\bigl(p\log p-p+1\bigr)\,dQ\\ &\ge 0, \end{aligned}$$
because
$$\int_{\mathcal X}(p-1)\,dQ=1-1=0.$$
If $D_{\mathrm{KL}}(P\\|Q)=0$, then
$$\int_{\mathcal X}\bigl(p\log p-p+1\bigr)\,dQ=0.$$
The integrand is nonnegative and vanishes only when $p=1$. Therefore,
$$p=1\qquad Q\text{-almost surely}.$$
Consequently, for every $A\in\mathcal F$,
$$P(A)=\int_Ap\,dQ=\int_A1\,dQ=Q(A),$$
so $P=Q$.

Conversely, if $P=Q$, then
$$\frac{dP}{dQ}=1\qquad Q\text{-almost surely},$$
and hence
$$D_{\mathrm{KL}}(P\|Q) =\int_{\mathcal X}\log 1\,dP =0.$$
Therefore,
$$D_{\mathrm{KL}}(P\|Q)\ge 0$$
...
>  where the supremum is taken over measurable extended-real-valued functions $f:\mathcal X\to[-\infty,\infty)$ such that $f$ is integrable under $P$ and $0<\mathbb E_Q[e^f]<\infty.$ 
>  
...
Let $S=\{ p>0 \}$, then $$\int_{\mathcal{X}} \frac{e^f}{p} dP=\int_{S} \frac{e^f}{p} dP=\int_{S} e^f dQ\leq\int_{\mathcal{X}} e^f dQ$$
and note $$\log \left( \int_{\mathcal{X}} e^f dQ \right) = \log \mathbb{E}_Q[e^f]$$
Thus
...
Note that from the derivation above, such $f^{*}$ is not unqiue, we can take any valid $c$ we want. We can take $c=1$.
`QED.`
...
> If we look at it, which is $$\log\int e^f\,dQ$$
> It has an unnormalized measure $$e^f\,dQ$$
> which has a total mass of $$Z_f:=\int e^f\,dQ=\mathbb E_Q[e^f]$$
> Normalize it we get a measure $$dQ_{f}=\frac{e^f}{Z_f}\,dQ$$
> Thus $$\log\frac{dQ_f}{dQ} = f-\log Z_f$$
> And so $$\mathbb{E}_{P}[f]-\log Z_f=\mathbb{E}_{P}[f-\log Z_f]=\mathbb{E}_{P}\left[ \log\frac{dQ_f}{dQ} \right]$$
...
$$Q_f(A) := \frac{1}{Z_f}\int_A e^{f(x)}\,dQ(x), \qquad A\in\mathcal F.$$
...
$$\begin{aligned} \frac{dP}{dQ_f} &= \frac{dP/dQ}{dQ_f/dQ}\\ &= \frac{p}{e^f/Z_f}\\ &= pZ_fe^{-f} \end{aligned}$$
$Q_{f}$-almost surely hence also $P$-almost surely because $P\ll Q_f$.

Taking logarithms,
$$\begin{aligned} \log\frac{dP}{dQ_f} &= \log\left(pZ_fe^{-f}\right)\\ &= \log p+\log Z_f-f. \end{aligned}$$
Therefore,
$$\begin{aligned} D_{\mathrm{KL}}(P\|Q_f) &= \mathbb E_P \left[ \log\frac{dP}{dQ_f} \right]\\ &= \mathbb E_P[\log p+\log Z_f-f]\\ &= \mathbb E_P[\log p] +\log Z_f -\mathbb E_P[f]\\ &= D_{\mathrm{KL}}(P\|Q) +\log Z_f -\mathbb E_P[f]. \end{aligned}$$
We obtain
$$\mathbb E_P[f] - \log\mathbb E_Q[e^f] = D_{\mathrm{KL}}(P\|Q) - D_{\mathrm{KL}}(P\|Q_f)$$
By the nonnegativity of KL divergence,
$$D_{\mathrm{KL}}(P\|Q_f)\ge0$$
Applying this gives
$$\mathbb E_P[f] - \log\mathbb E_Q[e^f] \le D_{\mathrm{KL}}(P\|Q)$$
Since this holds for every admissible $f$,
$$\sup_{f}\left\{  \mathbb{E}_P[f] - \log \mathbb{E}_Q[e^f] \right\}\leq D_{\mathrm{KL}}(P\|Q)$$
The equality holds precisely when
$$D_{\mathrm{KL}}(P\|Q_f)=0$$
Since
$$D_{\mathrm{KL}}(P\|Q_f)=0 \quad\Longleftrightarrow\quad Q_f=P$$
**if and only if**
$$\frac{dQ_f}{dQ} = \frac{dP}{dQ}$$
Therefore,
$$\frac{e^{f^\star}}{Z_{f^\star}} = p$$
Equivalently,
$$e^{f^\star} = Z_{f^\star}p$$
Taking logarithms gives
$$\begin{aligned} f^\star &= \log p+\log Z_{f^\star}\\ &= \log\frac{dP}{dQ} +\log Z_{f^\star} \end{aligned}$$
Since $\log Z_{f^\star}$ is a constant, every maximizer must have the form
$$f^\star = \log\frac{dP}{dQ}+C$$
Now we verify that this function attains the bound.
Fix $C\in\mathbb R$, then
$$e^{f^\star}=e^Cp$$
Hence
$$\begin{aligned} Z_{f^\star} &= \mathbb E_Q[e^{f^\star}]\\ &= \int_{\mathcal X}e^Cp\,dQ\\ &= e^C\int_{\mathcal X}\frac{dP}{dQ}\,dQ\\ &= e^C\int_{\mathcal X}dP\\ &= e^C \end{aligned}$$
Therefore
$$\frac{dQ_{f^\star}}{dQ} = \frac{e^{f^\star}}{Z_{f^\star}} = \frac{e^Cp}{e^C} = p = \frac{dP}{dQ}.$$
Thus
$$Q_{f^\star}=P$$
Substituting $f^\star$ gives
$$\begin{aligned} \mathbb E_P[f^\star] - \log\mathbb E_Q[e^{f^\star}] &= \mathbb E_P[\log p+C]-\log(e^C)\\ &= \mathbb E_P[\log p]+C-C\\ &= D_{\mathrm{KL}}(P\|Q). \end{aligned}$$
Consequently,
$$\sup_f \left\{ \mathbb E_P[f] - \log\mathbb E_Q[e^f] \right\} \geq D_{\mathrm{KL}}(P\|Q)$$
Combining above we conclude that
$$\sup_f \left\{ \mathbb E_P[f] - \log\mathbb E_Q[e^f] \right\} = D_{\mathrm{KL}}(P\|Q)$$
...
(It can be proven that $\mu_{0}\ll \mu_{1,P}$ just like what we did for $\mu_{1,\boldsymbol{P}^\star_\Delta} \ll \mu_0$, except we need $f_{1}>0$ almost everywhere, indeed $f_{1}(r)>0$ for $r\in(0,1)$)
...
For $P_{w}=0$, the corresponding term is defined as 0.
